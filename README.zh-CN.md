...
- 捕获文字、图片、截图、文件、颜色和二维码，在可搜索的 Liquid Glass「墙」中浏览，
  随时复制或粘贴任意保留条目。
- 搜索覆盖全部保留历史，包括复制的文字、OCR 文字、二维码内容、文件名与路径、颜色；
  可按内容类型、来源应用、标签或收藏过滤。
- 历史静态加密存储，密钥仅存于本机 macOS Keychain，永不同步、也不会进入备份。
- 可选的本机「图片文字自动索引」让新捕获图片中的文字可被搜索；新图片中的二维码
  始终自动索引。
- 保留时长可选 1 天、7 天、30 天、3 个月、6 个月、1 年或无限制；收藏与已打标签的
  条目不会被自动过期清理。
- 单条文本可就地编辑；设置中可查看历史的精确磁盘占用。
- 默认排除 Apple 密码与钥匙串访问；可继续排除其他应用，或忽略通过「通用剪贴板」
  到达的新条目，而不影响本机复制与既有历史。
...
- **剪贴板** 标签页：历史监听、粘贴行为（自动粘贴或仅复制）、含收藏 / 标签保护的保留时长、图片文字自动索引、来源应用与通用剪贴板排除、存储占用，以及清空历史。
...
- **同步** 标签页：可选的配置同步（共享文件夹或 WebDAV）、同步状态与配置指南，以及一次性的配置备份 / 恢复。
- **通用** 标签页：开机自启、语言、菜单栏图标样式、Hyper Key、命令面板快捷键、定时关机、辅助功能 / 自动化 / 屏幕录制权限状态及一键申请，以及自动更新配置。
...
让每台 Mac 指向云盘（Google Drive、Dropbox、iCloud Drive、NAS…）里的同一个文件夹，
或一个 WebDAV 目录（Nextcloud、坚果云、Synology…），AnyDoor 就会自动同步应用快捷键、
内置项偏好、快速入口和可携带设置——无需账号、无需服务器，容忍离线间隔。并发修改
自动合并：不同条目的改动都会保留，删除在所有机器上生效；每台 Mac 只写自己的状态
文件，云端客户端不会看到冲突写入。可选功能，默认关闭；机器本地状态（助手授权、
脚本插件、同步文件夹的选择本身）永不同步。

配置方法、同步范围、冲突语义与排障：[docs/config-sync.md](docs/config-sync.md)。

### 备份与恢复

- 把应用快捷键、内置项偏好、快速入口、可携带的剪贴板设置（标签、分类顺序、来源排除与「忽略通用剪贴板」偏好）、可携带的截图设置（保存位置、文件名、自动复制、自动保存、声音、延迟与浮层超时）、翻译偏好与服务定义、已安装插件集合，以及白名单内的通用设置导出为带版本号的快照，并在另一台 Mac 上导入。
- 剪贴板历史、翻译历史、API keys 和机器相关的键不会导出；导入时按 bundle ID 重新解析应用路径，改动无需重启即可生效。
- 多台 Mac 的持续同步请使用[配置同步](docs/config-sync.md)；备份保留为一次性迁移 / 分享工具。
