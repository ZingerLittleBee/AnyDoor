...
├── Models/                     # SwiftData @Model (these 6 + each plugin's modelSchemaTypes =
│                               #   the ModelContainer schema): KeyBinding / BuiltinPreference /
│                               #   ClipboardHistoryItem / HostProfile / TranslationRecord / Quicklink
│                               #   (ImageConversionRecord lives in the ImageConversionPlugin target).
...
- **Native Plugins (ADR-0005/0006/0007)**: first-party feature modules the user installs/uninstalls from Settings → 插件; code always ships (install is a logical state in UserDefaults key `plugins.installed`). `PluginRegistry` (`@MainActor @Observable`, `Services/Plugins/`) is the single seam: it owns the compile-time plugin list (built in AppDelegate with a `CorePluginHost`), exclusive claim lookups (`claimOwner`/`isAvailable`/`availableCommands`), and the transactional lifecycle — `install` activates and registers surfaces immediately; `uninstall` runs the plugin's throwing `deactivate()` (revert side effects, cancel in-flight work) **before** any state change, so a thrown error leaves the plugin fully installed. Uninstalled = invisible everywhere: `PanelStore.rebuild` drops unavailable commands (panel, palette, and Panel settings all derive from it), hotkey compilation drops their bindings, and the clipboard-history convert action gates on `isAvailable(.imageConversion)` (a PRD-registered debt site, like capture save-as importing the plugin's codec). Plugin user data (SwiftData rows, preference rows, hotkeys) is retained; reinstall restores it without relaunch. `BackupService.reconcileAfterImport` forwards to installed plugins via `PluginRegistry.reconcileAfterImport()`. The first pilot is `ImageConversionNativePlugin` (its module reaches the host only through `PluginHostServices`; module-internal access goes through the `PluginHost` bridge + its own `L10n` key enum resolving against the shared catalog).
...
- **Config sync / backup**: backup (Settings → General) serializes app shortcuts (`KeyBinding`), builtin preferences (`BuiltinPreference`), quicklinks (`Quicklink`, full rows incl. keyword/hotkey/open-with; schema v2), and whitelisted general settings (`SyncSettingsRegistry`) into a schema-versioned Codable `BackupSnapshot`. Clipboard history, machine-specific keys, and the favicon cache are excluded, `appPath` is never serialized (re-resolved from bundle ID on import), import merges per key (imported wins, local-only rows kept; quicklinks merge by `id`, and a keyword collision clears the differently-id'd local row's keyword so uniqueness holds), and `reconcileAfterImport` re-reads settings into CommandPaletteService / LocalizationManager / HyperKeyService / ScheduledShutdownService / CaptureSettings / TranslationSettings, reloads QuicklinkStore, and rebuilds hotkey snapshots so changes apply without relaunch. Storage sits behind a `SyncBackend` protocol (current backend: `LocalFileBackend`).
