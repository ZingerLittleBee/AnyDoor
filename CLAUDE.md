...
└── Views/
    ...
    │                Panel + Quicklinks + Clipboard + Capture + Translation + General, traffic lights inside the
...
- **The menu bar is not MenuBarExtra**: the menu-bar item is owned by the AppKit `MenuBarController` (`NSStatusItem` + a floating `NSPanel`). SwiftUI `MenuBarExtra` with `isInserted: false` infinite-loops the scene graph on macOS 26, so `AnyDoor.swift` keeps only the `Settings` scene. When AppKit needs to open Settings it goes through `SettingsOpener` (an invisible 1×1 `NSHostingView` mounted at launch captures the `\.openSettings` closure).
