...
- **HotkeyAction dispatch**: `HotkeyCoordinator` (`@MainActor`) compiles snapshots from `KeyBinding` + `BuiltinPreference` + `Quicklink` rows plus the Command Palette hotkey via the pure static `compile(bindings:prefs:quicklinks:paletteHotkey:)`; `dispatch(_:)` routes matched actions (`openQuicklink` opens a plain Link directly, or summons the palette in argument-input mode for a Search Template). HotkeyService's callback uses an injected `dispatcher` closure, bound in `AppDelegate.applicationDidFinishLaunching` to `HotkeyCoordinator.shared.dispatch`. Do not reference PanelStore or HotkeyCoordinator directly inside HotkeyService — keep HotkeyService decoupled from business logic.
