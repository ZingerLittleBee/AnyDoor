...
- **Command palette second-level menu**: option-bearing commands drill into a second level instead of acting with a default. `CommandPaletteOptions` (`@MainActor`) is the single source of truth for which builtins are option parents (`keepAwake` / `scheduledShutdown` / `brightness` / `hostsManager`) and their options; pure per-item builders take fetched state (`isOn` / `isArmed` / `displays` / `profiles`) so they unit-test without singletons. `CommandPaletteState` holds a `.root` ⇄ `.options` stack; option rows reuse `PanelEntry` via `Source.paletteOption(id:)` (only a `String`, the action looked up by id on the MainActor). `CommandPaletteWindowController` lists Brightness (only with an external DDC display) and Hosts, drills in on commit, runs an option then closes, and pops on `Esc` / empty-query `Backspace`.
