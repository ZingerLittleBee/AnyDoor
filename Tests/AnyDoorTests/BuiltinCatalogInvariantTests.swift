...
struct BuiltinCatalogInvariantTests {
    ...
    @MainActor
    private static func providersByItem() -> [BuiltinItem: any BuiltinProvider] {
        var byItem: [BuiltinItem: any BuiltinProvider] = [:]
        for provider in BuiltinProviderRegistry.makeAll(onKeepAwakeChange: { _ in }) {
            byItem[provider.itemKey] = provider
        }
        return byItem
    }

    @Test @MainActor func providerItemKeysAreUnique() {
        let providers = BuiltinProviderRegistry.makeAll(onKeepAwakeChange: { _ in })
        let keys = providers.map(\.itemKey)
        #expect(keys.count == Set(keys).count, "duplicate provider registrations for the same BuiltinItem")
    }

    @Test @MainActor func everyToggleItemHasAToggleProvider() {
        let byItem = Self.providersByItem()
    ...
    @Test @MainActor func everyActionItemHasAnActionProvider() {
        let byItem = Self.providersByItem()
    ...
    @Test @MainActor func nonActionableKindsHaveNoProvider() {
